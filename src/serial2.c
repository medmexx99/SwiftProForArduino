...
uint8_t serial2_rx_buffer[RX2_BUFFER_SIZE];
uint8_t serial2_rx_buffer_head = 0;
volatile uint8_t serial2_rx_buffer_tail = 0;

uint8_t serial2_tx_buffer[TX2_BUFFER_SIZE];
...
uint8_t serial2_get_tx_buffer_count()
{
  ...
  return (TX2_BUFFER_SIZE - (ttail-serial2_tx_buffer_head));
}
...
//  UCSR1B |= 1<<RXCIE1;
  ...
  // Set baud rate
  #if SERIAL2_BAUD_RATE < 57600
    uint16_t UBRR2_value = ((F_CPU / (8L * SERIAL2_BAUD_RATE)) - 1)/2 ;
    UCSR2A &= ~(1 << U2X2); // baud doubler off  - Only needed on Uno XXX
  #else
    uint16_t UBRR2_value = ((F_CPU / (4L * SERIAL2_BAUD_RATE)) - 1)/2;
...
void serial2_write(uint8_t data) {
  ...
  if (next_head == TX2_BUFFER_SIZE) { next_head = 0; }
...
ISR(USART2_UDRE_vect)
{
  ...
  { 
    ...
    tail++;
    if (tail == TX2_BUFFER_SIZE) { tail = 0; }
...
uint8_t serial2_read()
{
  ...
  if (serial2_rx_buffer_head == tail) {
    ...
    tail++;
    if (tail == RX2_BUFFER_SIZE) { tail = 0; }
    serial2_rx_buffer_tail = tail;

    #ifdef ENABLE_XONXOFF
      if ((serial2_get_rx_buffer_count() < RX_BUFFER_LOW) && flow_ctrl == XOFF_SENT) {
        flow_ctrl = SEND_XON;
				switch( current_uart ){
					case UART0:		UCSR0B |=  (1 << UDRIE0); // Force TX
						break;
					case UART1:		UCSR1B |=  (1 << UDRIE1); // Force TX
						break;
...
//    default: // Write character to buffer
      ...
      if (next_head != serial2_rx_buffer_tail) {
        ...
        #ifdef ENABLE_XONXOFF
          if ((serial2_get_rx_buffer_count() >= RX_BUFFER_FULL) && flow_ctrl == XON_SENT) {
            flow_ctrl = SEND_XOFF;
